...
The North Star for code generation, validation, and delivery in this repo. Drop this file at the
top of any project. It is the method; the project's own documents are the content.

## 1. The idea

A **Behaviour Contract (BC)** is a written, negotiated agreement about *observable behaviour* for
one slice of a system: the routes, the inputs, the success and failure results, what survives a
...
```
<project>.design.md          product, domain, flows, guardrails, iteration plan
...
| --- | --- | --- |
| `*.design.md` | What are we building and why? What is true of the product? | Route-level detail, status codes |
| `*.bc.md` | What does a caller observe? | Class names, libraries, schema |
| `*.spec.md` | How is it built? | Behaviour, status codes, product rules |
...
One repo has one design doc and many BCs. A BC has zero or more specs. Most BCs need no spec at all.

## 3. Precedence, and the amendment protocol
...
3. **Amend that document first**, in its own visible step.
...
- **Header** — which iteration, and one line stating which behaviour in the design doc it
  implements. A BC with no matching design-doc behaviour means one of the two is wrong.
...
- **Host / surface** — where the behaviour is reachable, including how tests reach it.
...
- **The API, case by case** — for each route: method, body, the success result, and *every* failure
  with its real status code and stable error code.
- **Rules** — invariants a reader could not infer from the route list.

Non-negotiables:

- **Real status codes and stable error strings.** A sketch is a sketch; the BC carries what the
  service actually returns. Error codes (`email_taken`, `storage_unavailable`) are part of the
...
## 5. A BC ships with a scaffold

This is the part that makes BCDD more than documentation.

**Landing a BC means landing the BC file *and* a minimum implementation** — every route, wired and
reachable, with real request/response shapes, real authentication and validation, and service stubs
that do nothing but return an agreed *unimplemented* signal.

The scaffold must:

- **Compile, boot, and route.** Every path in the BC exists and is reachable.
- **Return one agreed signal for unimplemented behaviour.** Default: HTTP **501** with
  `{ "error": "not_implemented" }`. Non-HTTP equivalents: a distinct `NotImplemented` error type, or
  a documented sentinel return. One signal, used everywhere, never a plausible-looking fake result.
- **Carry the real shapes.** Request and response types match the BC so tests compile against them.
...
assertion, or on the 501 — never on a compile error, a routing 404, or a null reference. That
distinction is the entire difference between "not built yet" and "the contract is wrong", and it is
what lets a test author and an implementer work at the same time without blocking each other.

**The stub signal is a debt marker, and it is tracked.** A slice may not be called done while any
`not_implemented` remains in it. Enforce it mechanically: a check that fails the build when a BC
...
| Either | proposing an amendment (§3) | amend silently |

Rules that keep the seam clean:

- **Tests bind to the contract, not the implementation.** They call the surface the BC describes.
  A test that reaches inside a service will break on every refactor and is not testing the contract.
...
- **The BC is the only shared mutable artifact**, so changing it is an announced act (§3).
- **"Done" is defined by the BC's case list**, not by either party's judgement. Both sides are
  checking against the same enumerated cases, which is why the case list must be exhaustive.
...
## 7. Specs — only when *how* needs agreeing
...
configuration and where it comes from, local setup steps, the test topology, and the list of cases
the slice must prove.

A spec never restates behaviour. If a status code appears in both a BC and its spec, delete it from
the spec. **Duplicated facts drift**, and the copy that drifts is always the one you read.

Skip the spec when the slice only adds routes over machinery already decided. Most do.

## 8. Test docs — only for a handoff

Write a `*.test.md` when the test work goes to a *different* agent or person than the
implementation, and only then. It carries the case list derived from the BC, the fixture and
...
## 9. Guardrails belong in the design doc
...
## 10. Iterations and definition of done

The design doc carries an iteration plan, and **an iteration is a set of BCs**. Each BC is listed
with its status. Slices small enough to finish beat slices that look tidy.

A **BC is done** when:

- every case in it has a test, and those tests pass in CI
- no `not_implemented` signal remains in its surface
- the BC, its specs, and the code agree — including any amendment made along the way
- the guardrails it touches are still true

An **iteration is done** when every BC in it is done and the design doc says so.

CI is where the contract stops being a promise. It runs the full suite on every push, plus whatever
checks catch drift that green tests cannot see — a stale migration, a stub left in a finished
slice, a doc that no longer matches. Never make a change look green by removing what proves it.
...
- **Sketch status codes in a contract.** Placeholders from a first draft become the thing an
  implementer builds.
...
## 12. Starting a new repo

1. Write `<project>.design.md`: the product, the domain objects, the flows, the guardrails, and an
   iteration plan naming the BCs.
2. Copy this file to the repo root.
3. For the first iteration, write each BC under `bc/<slice>/<slice>.bc.md`, each stating which
   design-doc behaviour it implements.
4. Add a spec only for slices where the mechanism needs deciding.
5. Land the scaffold for each BC: routes, shapes, auth, validation, stubs returning
   `not_implemented`.
6. Split the work — implementation and tests, in parallel if you have the hands for it.
7. Set up CI: full suite on every push, plus drift checks.
8. When an assumption breaks, amend the document first (§3).
9. Keep the README pointing at the design doc rather than restating it.

## 13. A BC in miniature
...
Implements `<project>.design.md` §3.1 "a person can sign in on several devices".
...
### POST /auth/login
- Body: `{ email, password }`
- Success: **200** + `{ id, email, token }`; a new session is created even if others exist
- Fail (unknown email or wrong password): **401** `{ error: "invalid_credentials" }`; no token
- Fail (store unreachable): **503** `{ error: "storage_unavailable" }` — never 401

### POST /auth/logout
- Header: `Authorization: Bearer {token}`
- Success: **204**; that session ends and that token never works again
- Fail (missing, expired, or already-revoked token): **401**

## Rules
- Sign-out ends only the presented session; other devices stay signed in
- A revoked session stays revoked across a restart
- Sessions are authoritative in the datastore, never in process memory
```
